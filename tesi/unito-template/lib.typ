...
#let template(
  ...
  // Configure raw text/code blocks (fallback to Windows monospace fonts)
  ...
  // Top-level entries in bold, skip special styling for References
  ...
  // Level-3 entries slightly smaller
  show outline.entry.where(level: 3): it => {
    text(size: 0.8em, it)
  }

  // 0.13+: indent takes `auto` or a length/function (boolean removed)
